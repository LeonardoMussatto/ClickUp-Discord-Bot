...
exports.run = async (client, message, args) => {
  ...
  if (task.err)
    return message.channel.send(`ittim threw out the paper work... ${task.err}`)
  const list = await fetch(
    `https://api.clickup.com/api/v2/list/${task.list.id}`,
    {
      method: "GET",
      headers: {
        Authorization: client.config.ClickUp_token,
  ...
  if (list.err)
    return message.channel.send(`ittim threw out the paper work... ${list.err}`)

  const status = list.statuses.find(
    (x) => x.status.toLowerCase() === newArgs[0]
  )
  if (!status)
    return message.channel.send(
      "Status not found! Ensure you gave me the correct status name"
    )
  ...
  if (res.err)
    return message.channel.send(`ittim threw out the paper work... ${res.err}`)
